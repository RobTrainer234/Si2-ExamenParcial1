...
@Component({
  ...
  template: `
     <header class="site-header"><button class="menu-toggle" type="button" (click)="openMenu()" aria-label="Abrir menú de catálogo"><span class="material-symbols-outlined">menu</span></button><a class="brand" routerLink="/catalog"><span>F</span><strong>FashionStore</strong><small>Catálogo</small></a><nav><a routerLink="/catalog" routerLinkActive="active">Colección</a>@if (auth.currentUser()?.role === 'ADMIN') { <a routerLink="/admin" routerLinkActive="active">Administración</a><a routerLink="/audit" routerLinkActive="active">Bitácora</a> } @if (auth.hasPermission('suppliers.manage')) { <a routerLink="/supply" routerLinkActive="active">Abastecimiento</a> } @if (auth.hasPermission('inventory.read') && auth.currentUser()?.role !== 'ADMIN') { <a routerLink="/operations" routerLinkActive="active">Operación</a> } @if (auth.hasPermission('suppliers.portal')) { <a routerLink="/supplier" routerLinkActive="active">Proveedor</a> } @if (auth.isAuthenticated()) { <span class="welcome">Hola, {{ auth.currentUser()?.first_name }}</span><button class="text-button" (click)="logout()">Salir</button> } @else { <a routerLink="/login">Ingresar</a><a class="nav-cta" routerLink="/register">Crear cuenta</a> }</nav></header>
