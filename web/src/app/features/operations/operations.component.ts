...
@Component({
  ...
  template: `
    <section class="operations-page">
      ...
      @if (!loading && items.length) { <div class="operations-table-card"><div class="table-wrap"><table><thead><tr><th>Producto</th><th>Variante</th><th>Sucursal</th><th>Stock</th><th>Estado</th><th></th></tr></thead><tbody>@for (item of items; track item.id) { <tr><td><strong>{{ item.product_name }}</strong><small>{{ item.sku }}</small></td><td>{{ item.size_name }} · {{ item.color_name }}</td><td>{{ item.branch_name }}</td><td>@if (canAdjust) { <input class="stock-input" type="number" min="0" [value]="draftValue(item)" (input)="setDraft(item.id, $any($event.target).value)"> } @else { <strong>{{ item.stock_quantity }}</strong> }</td><td><span class="status-pill" [class.active]="item.stock_quantity > 0">{{ item.stock_quantity > 0 ? 'Disponible' : 'Agotado' }}</span></td><td>@if (canAdjust) { <button class="small-button" type="button" (click)="save(item)">Guardar</button> }</td></tr> }</tbody></table></div></div> }
    </section>
  `,
})
export class OperationsComponent {
  ...
  save(item: InventoryRow): void {
    ...
    this.api.updateInventory(item.id, value).subscribe({ next: (updated) => { item.stock_quantity = updated.stock_quantity; item.available = updated.available; delete this.drafts[item.id]; }, error: () => this.error = true });
  }
